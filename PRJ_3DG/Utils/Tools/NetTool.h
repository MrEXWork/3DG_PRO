 ...
 */
+ (void)httpPostFileDataRequest:(NSString *)url WithFileFormdata:(NSMutableDictionary *)data withFormdaya:(NSMutableDictionary *)formData WithSuccess:(int (^)(Response *response))success failure:(void (^)(NSError *error))failure;

/**
 上传文件接口:以文件名的形式
 @param {"data":[nameaddr1,nameaddr2],"formate":"png","key":"photo","name":"photo.png"}
 */
+ (void)httpPostFileAddrRequest:(NSString *)url WithFileFormdata:(NSMutableDictionary *)data withFormdaya:(NSMutableDictionary *)formData WithSuccess:(int (^)(Response *response))success failure:(void (^)(NSError *error))failure;
@end
